...
import { Crown, Trash2, LogIn, LogOut, User, Sparkles, Zap, Menu, Plus } from 'lucide-react';

import { Toaster } from 'sonner';

function App() {
  const { 
    messages, isLoading, isReady, models, selectedModel, setSelectedModel, 
    sendMessage, clearChat, mode, setMode,
  ...
  return (
    <div className="flex min-h-screen bg-[#FAFAFA] font-sans text-gray-900 selection:bg-[#FFD700]/30 selection:text-black">
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${isSidebarOpen ? 'lg:pl-[280px]' : ''}`}>
        {/* Header */}
          ...
          <div className="max-w-4xl mx-auto h-full px-4 sm:px-6 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <motion.button
                ...
                className="p-2 rounded-full text-gray-500 transition-all hover:text-orange-500"
                title="New Chat"
              >
                <Plus size={20} />
              </motion.button>
        ...
        <main className="flex-1 flex flex-col max-w-4xl w-full mx-auto relative">
          <AnimatePresence mode="wait">
            {messages.length === 0 ? (
                ...
                <div className="space-y-3 max-w-lg">
                  ...
                  <div className="flex items-center justify-center gap-2 mt-2">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest ${
                      mode === 'math' ? 'bg-orange-100 text-orange-600' :
              ...
              <MessageList key={currentSessionId} messages={messages} isLoading={isLoading} mode={mode} />
            )}
          </AnimatePresence>
        </main>

        {/* Input Area */}
        <InputArea onSend={sendMessage} isLoading={isLoading} isReady={isReady} mode={mode} />
      </div>
    </div>
  );
}
